...
if ! "$cygwin" && ! "$Darwin" && ! "$msys" ; then
    ...
    if [ "$MAX_FD" = "maximum" ] && [ -n "$MAX_FD_LIMIT" ] ; then
        ulimit -n "$MAX_FD_LIMIT" || warn "Could not set maximum file descriptor limit"
    fi
fi
...
# Collect all arguments for the java command; use the shell's word splitting rules to split the command into arguments
